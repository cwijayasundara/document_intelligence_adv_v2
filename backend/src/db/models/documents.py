...
class Document(TimestampMixin, Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, server_default=text("'uploaded'"), index=True
    )
    ...
    # Pipeline tracking
    pipeline_node_status: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True, server_default=text("'{}'")
    )
    pipeline_thread_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
