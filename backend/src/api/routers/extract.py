...
from src.db.models import ExtractedValue
...
def _get_extraction_service() -> ExtractionService:
    ...
    if _extraction_service is None:
        ...
        _extraction_service = ExtractionService(extraction_dir=settings.storage.extraction_dir)
    return _extraction_service


def _values_to_items(values: list[ExtractedValue]) -> list[ExtractionResultItem]:
    """Convert DB ExtractedValue records to API response items."""
    return [
        ExtractionResultItem(
            id=v.id,
            field_name=v.field.field_name if v.field else "",
            display_name=v.field.display_name if v.field else "",
...
@router.post("/extract/{doc_id}", response_model=ExtractionResponse, summary="Extract fields")
async def extract_document(
    ...
    """Extract fields from a document. Returns cached results unless force=true."""
    doc_repo = DocumentRepository(session)
    doc = await doc_repo.get_by_id(doc_id)
    if doc is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")

    # Fast path: return existing DB results if not forced
    if not force:
        ev_repo = ExtractedValuesRepository(session)
        existing = await ev_repo.get_by_document_id(doc_id)
        if existing:
            logger.info("Returning cached extraction for %s (%d fields)", doc_id, len(existing))
            items = _values_to_items(existing)
            review_count = sum(1 for v in existing if v.requires_review and not v.reviewed)
    ...
    if not await run_guard.acquire(str(doc_id)):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Already being processed")
    try:
        try:
        ...
        if not doc.parsed_path or not Path(doc.parsed_path).exists():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No parsed content")

        async with aiofiles.open(doc.parsed_path, "r") as f:
        ...
        logger.info("Saved %s: %d results, %d need review", doc_id, len(saved), review_count)
        ...
        items = [
            ExtractionResultItem(
                id=saved[i].id, field_name=results[i]["field_name"],
...
async def get_extraction_results(
    ...
    """Get extraction results for a document from DB."""
    doc_repo = DocumentRepository(session)
    if await doc_repo.get_by_id(doc_id) is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")

    ev_repo = ExtractedValuesRepository(session)
    values = await ev_repo.get_by_document_id(doc_id)
    if not values:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No extraction results")

    items = _values_to_items(values)
    review_count = sum(1 for v in values if v.requires_review and not v.reviewed)
    return ExtractionResultsResponse(
        document_id=doc_id, results=items,
        requires_review_count=review_count, all_reviewed=review_count == 0,
    )
...
async def update_extraction_results(
    ...
    ev_repo = ExtractedValuesRepository(session)
    updated_count = await ev_repo.update_values([u.model_dump() for u in body.updates])
...
async def _load_extraction_fields(session: AsyncSession, category_id: uuid.UUID | None) -> list[dict]:
    ...
    return [
        {
            "field_id": f.id, "field_name": f.field_name, "display_name": f.display_name,
            "description": f.description, "data_type": f.data_type,
            "examples": f.examples, "required": f.required,
        }
        for f in fields
    ]
